...
def judge_output(output_text):
    ...
    for criterion, meta in JUDGE_CRITERIA.items():
        ...
        if criterion == "monetization_path_clear":
            ...
            hit = any(
                kw in output_text.lower()
                for kw in ["failure-case", "disconfirming", "adversarial", "kill", "survived"]
            )
            ...
            hit = any(
                kw in output_text.lower()
                for kw in ["single next action", "today", "physical action"]
            )
        elif criterion == "knightian_uncertainty_classified":
            hit = any(
                kw in output_text.lower()
                for kw in ["knightian uncertainty", "known", "risk", "knightian"]
            )
